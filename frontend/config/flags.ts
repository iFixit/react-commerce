export const flags = {
   STORE_PAGES_APP_ROUTER_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__STORE_PAGES_APP_ROUTER_ENABLED === 'true',
   PRODUCT_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__PRODUCT_PAGE_ENABLED === 'true',
   STORE_HOME_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__STORE_HOME_PAGE_ENABLED === 'true',
   TROUBLESHOOTING_COLLECTIONS_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__TROUBLESHOOTING_COLLECTIONS_ENABLED ===
      'true',
   APP_ROUTER_PRODUCT_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__APP_ROUTER_PRODUCT_PAGE_ENABLED === 'true',
   APP_ROUTER_PARTS_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__APP_ROUTER_PARTS_PAGE_ENABLED === 'true',
   APP_ROUTER_TOOLS_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__APP_ROUTER_TOOLS_PAGE_ENABLED === 'true',
   APP_ROUTER_MARKETING_PAGE_ENABLED:
      process.env.NEXT_PUBLIC_FLAG__APP_ROUTER_MARKETING_PAGE_ENABLED ===
      'true',
};
