...
test.describe('Cross-sell test', () => {
   ...
   test('Current item from cross-sell can be added to cart', async ({ page }) => {
      ...
      for (let i = 0; i < count; i++) {
         ...
         const isCurrent = await product.locator('span', { hasText: 'Current item' }).isVisible();
         if (isCurrent) {
            currentProductPrice = await product.getByTestId('product-price').textContent();
            await expect(currentProductPrice).toMatch(/^\$[0-9]+(\.[0-9]{2})/);
            currentProductTitle = await product.getByTestId('cross-sell-item-title').textContent();
            continue;
         }

         // Deselect other cross-sell products
         otherProductTitles.push(await product.getByTestId('cross-sell-item-title').textContent());
      ...
      await expect(page.getByText('Total Price: ' + currentProductPrice)).toBeVisible();

      // Assert adding to cart only adds current product
      await page.getByTestId('cross-sell-add-to-cart-button').click();

      const cartDrawerText = await page.getByTestId('cart-drawer-body').textContent();
   ...
   test('Cross-sell products can be added to cart', async ({ page }) => {
      ...
      for (let i = 0; i < count; i++) {
         ...
         allProductTitles.push(await product.getByTestId('cross-sell-item-title').textContent());
         expectedTotalPrice += parseFloat((await product.getByTestId('product-price').textContent()).slice(1));
      }

      // Assert total price matches the sum of all products
      await expect(page.getByText('Total Price: $' + expectedTotalPrice)).toBeVisible();
